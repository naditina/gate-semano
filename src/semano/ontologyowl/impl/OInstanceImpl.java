 ...
 *
 *  $Id: OInstanceImpl.java 11884 2009-11-03 17:21:45Z johann_p $
 */
   ...
   */
    ...
    public List<OResource> getRDFPropertyValues(RDFProperty aProperty) {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            Property prop = ontologyService.getRDFProperty(
                    list[i].getUri());
            if (prop == null) {
    ...
    public List<Literal> getDatatypePropertyValues(DatatypeProperty aProperty) {
        try {
            ...
            for (int i = 0; i < values.length; i++) {
                ...
                // dataType
                if (dataType == null || !dataType.isValidValue(values[i].getValue())) {
                    throw new InvalidValueException("The value :\"" + values[i].getValue()
                            + "\" is not compatible with the dataType \""
                            + values[i].getDatatype() + "\"");
                }
                list.add(new Literal(values[i].getValue(), dataType));
            }
            return list;
